...
export default function Input() {
  ...
  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: (userInput.length < 10) ? 'royalblue' : 'crimson', /* STEP 2 */
  };

  return (
    <div className='widget-input container'>
      <h2>Input</h2>
      <div id='output' style={style}>{userInput.toUpperCase()}</div> {/* STEP 3 */}
      <div>
        <input id='input' type='text' onChange={changeInput} value={userInput} /> {/* STEP 6 */}
