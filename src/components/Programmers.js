...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    for (let i = 0; i < listOfAwesome.length; i += 1)
      {
        let currentProgrammerData = listOfAwesome[i];

        if (currentProgrammerData.id === selectedProgrammerID)
          { return currentProgrammerData.name; }
      }
  };
      ...
      <div className='programmers'>
        {
          ...
          programmerNames.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => setSelectedProgrammerID(dev.id)}>Feature</button>
            </div>
          )
        }
      </div>
